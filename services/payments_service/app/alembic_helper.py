...
async def run_alembic_migrations(database_dsn: str) -> None:
    try:
        ...
        logger.info("🚀 Running Alembic migrations (payments)...")
        command.upgrade(alembic_cfg, "head")
        logger.info("✅ Alembic migrations applied successfully (payments).")
    except Exception as e:
        logger.error(f"❌ Alembic migration failed (payments): {e}")
        raise
