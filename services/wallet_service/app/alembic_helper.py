...
async def run_alembic_migrations(database_dsn: str) -> None:
    try:
        ...
        logger.info("🚀 Running Alembic migrations (wallet)...")
        command.upgrade(alembic_cfg, "head")
        logger.info("✅ Alembic migrations applied successfully (wallet).")
    except Exception as e:
        logger.error(f"❌ Alembic migration failed (wallet): {e}")
        raise
